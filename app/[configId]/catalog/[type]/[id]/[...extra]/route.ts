...
export async function GET(
  ...
  {
    params,
  }: { params: Promise<{ configId: string; type: string; id: string; extra: string[] }> },
) {
  const { configId, type, id, extra } = await params;
  ...
  return handleStremioCatalogRequest(request, configId, type, id, extra);
}
